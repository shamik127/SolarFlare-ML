...
def rescale_write():
    ...
    for i in range(len(ALL_FILES)):
        ...
        if (np.isnan(IMAGE).any()):
            continue
        """
        NEW_IMAGE = transform.resize(IMAGE.astype(float),(128,128))
        NEW_IMAGE = np.round(NEW_IMAGE,2)
        ...
        WRITE_PATH = os.path.abspath("../shared/Data/HMI_LOS_SHARPS/valid_magnetograms/los3")
